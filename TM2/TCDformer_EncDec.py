...
class moving_avg(nn.Module):
    def __init__(self, kernel_size, stride):
        ...
        self.to(self.device)  # 确保模块参数在正确设备
...
class series_decomp_multi(nn.Module):
    def __init__(self, kernel_list):
        ...
        # 关键修复：使用ModuleList注册子模块
        self.moving_avg = nn.ModuleList([
            moving_avg(kernel, stride=1).to(self.device)
            for kernel in kernel_list
        ])
        self.layer = nn.Linear(1, len(kernel_list)).to(self.device)
    ...
    def forward(self, x):
        ...
        weights = self.softmax(self.layer(x.unsqueeze(-1)))
        moving_mean = torch.sum(moving_mean * weights, dim=-1)
        res = x - moving_mean
        print(f"[Decomp] res shape: {res.shape}, moving_mean shape: {moving_mean.shape}")
        return res, moving_mean  # 确保返回两个值
...
class series_decomp(nn.Module):
    def __init__(self, kernel_size):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.moving_avg = moving_avg(kernel_size, stride=1).to(self.device)
