from TCDformer_EncDec import EncoderLayer, Encoder, DecoderLayer, Decoder, AttentionLayer, series_decomp, series_decomp_multi
import torch.nn as nn
import torch
from Embed import DataEmbedding
from Attention import WaveletAttention, FourierAttention, FullAttention
from RevIN import RevIN
from LLSA import LLSA
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        print(f"x_enc shape: {x_enc.shape}, x_mark_enc shape: {x_mark_enc.shape}")
        print(f"x_dec shape: {x_dec.shape}, x_mark_dec shape: {x_mark_dec.shape}")
        ...
        print(f"seasonal_enc shape: {seasonal_enc.shape}, trend_enc shape: {trend_enc.shape}")
        print(f"seasonal_dec shape after padding: {seasonal_dec.shape}")

        # seasonal
        enc_out = self.enc_seasonal_embedding(seasonal_enc, x_mark_enc)
        print(f"enc_out shape after enc_seasonal_embedding: {enc_out.shape}")
        enc_out, attn_e = self.seasonal_encoder(enc_out, attn_mask=enc_self_mask)
        print(f"enc_out shape after seasonal_encoder: {enc_out.shape}")

        dec_out = self.dec_seasonal_embedding(seasonal_dec, x_mark_dec)
        print(f"dec_out shape after dec_seasonal_embedding: {dec_out.shape}")
        ...
        print(f"seasonal_out shape after seasonal_decoder: {seasonal_out.shape}")
        ...
        print(f"seasonal_ratio shape: {seasonal_ratio.shape}")
        ...
        print(f"trend_out shape: {trend_out.shape}")

        # final
        dec_out = trend_out + seasonal_ratio * seasonal_out
        print(f"dec_out shape (final output): {dec_out.shape}")
